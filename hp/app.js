...
const Wallpaper = () => {
    return (
	<img src="https://server.wallpaperalchemy.com/storage/wallpapers/92/windows-xp-wallpaper-bliss-4k-wallpaper.jpeg" className="absolute inset-0 bg-cover bg-center h-full w-full" />
    );  
};
...
const DesktopApps = () => {
	...
	{apps.map((app) => (
	  ...
	  >
	    <div className="w-14 h-14 rounded bg-white/5 backdrop-blur-sm flex items-center justify-center group-hover:bg-white/20 group-hover:shadow-lg transition-all border border-white/0 group-hover:border-white/30">
	      <img src={app.icon} alt={app.name} className="w-8 h-8" />
	    </div>
	    <span className="text-xs text-white mt-1 text-center drop-shadow-md bg-black/40 px-2 py-0.5 rounded group-hover:bg-black/60 transition">
	      {app.name}
	    </span>
	  </a>
	))}
...
const ClockWidget = () => {
  ...
  return (
    <div className="ml-auto flex items-center gap-2 text-white/90 text-xs px-2 h-full">
...
const MainWindow = () => {
	...
	<div className="bg-gradient-to-r from-[#1c4b7a] to-[#3a7bb5] px-3 py-2 flex justify-between items-center">
	    <span className="text-white text-sm font-bold tracking-wide hover:animate-bounce">@lyraplasma | Saintess of GNU Emacs</span>
	  <div className="flex gap-2">
	    <div className="w-3 h-3 bg-gradient-to-r from-yellow-400 to-yellow-800 rounded-sm shadow-inner"></div>
	    <div className="w-3 h-3 bg-green-500 rounded-sm shadow-inner"></div>
	    <div className="w-3 h-3 bg-red-500 rounded-sm shadow-inner"></div>
	  </div>
	</div>
